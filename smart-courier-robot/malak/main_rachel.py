...
try:
    ...
    while True:
        ...
        if error < -DEADBAND:
            ...
            left_speed = FORWARD_SPEED + (Kp * diff)
            
            right_speed = FORWARD_SPEED - (Kp * diff)
            

                # keep speeds within safe range
            max_speed = POWER_LIMIT
            left_speed = clamp(left_speed, -max_speed, max_speed)
            my_string = "left speed: "
            result = f"{my_string}{left_speed}"
            print(result)
            right_speed = clamp(right_speed, -max_speed, max_speed)
            my_string = "right speed: "
            result = f"{my_string}{right_speed}"
            print(result)
